...
export const ChatInput: React.FC<ChatInputProps> = ({ onSend, onImagePick, isCentered = false }) => {
  ...
  const ActionBtn = ({ label, icon, disabled = false, onPress }: any) => (
    ...
    >
        ...
        <Text style={styles.actionLabel}>{label}{disabled ? " (Скоро)" : ""}</Text>
    </TouchableOpacity>
  );
    ...
    <View style={isCentered ? styles.centeredContainer : styles.bottomContainer}>
          ...
          <View style={styles.bottomRow}>
            <View style={styles.leftActions}>
              ...
              <TouchableOpacity style={styles.iconButton} onPress={() => setThinkingEnabled(!isThinkingEnabled)}>
                <Svg width="22" height="22" fill={isThinkingEnabled ? colors.accent : "none"} stroke={isThinkingEnabled ? colors.accent : colors.text.tertiary} viewBox="0 0 24 24">
      ...
      <Modal visible={isMenuOpen} transparent animationType="fade" onRequestClose={() => setIsMenuOpen(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setIsMenuOpen(false)}>
            <View style={[styles.menuContainer, isCentered ? styles.menuCentered : styles.menuBottom]}>
                <ActionBtn 
                    label="Фото" 
                    onPress={onImagePick}
                    icon={<Svg width="20" height="20" fill="none" stroke="white" viewBox="0 0 24 24"><Path strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></Svg>} 
                />
                <ActionBtn 
                    label="Аудио" 
                    disabled 
                    icon={<Svg width="20" height="20" fill="none" stroke="white" viewBox="0 0 24 24"><Path strokeWidth="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></Svg>} 
                />
                <ActionBtn 
                    label="Документы" 
                    disabled 
                    icon={<Svg width="20" height="20" fill="none" stroke="white" viewBox="0 0 24 24"><Path strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></Svg>} 
                />
            </View>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  bottomContainer: { padding: 16, backgroundColor: colors.background, borderTopWidth: 1, borderTopColor: colors.border },
  centeredContainer: { width: '100%', maxWidth: 540, marginTop: 40, paddingHorizontal: 16 },
  inputWrapper: { backgroundColor: colors.surface, borderRadius: 28, borderWidth: 1, borderColor: colors.border, padding: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.15, shadowRadius: 15, elevation: 10 },
  input: { fontFamily: fonts.regular, fontSize: 16, color: colors.text.primary, minHeight: 36, paddingHorizontal: 16, paddingVertical: 8, textAlignVertical: 'top' },
  ...
  modelPickerBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#111111', borderWidth: 1, borderColor: colors.border, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, gap: 4, maxWidth: 120 },
  modelPickerText: { color: colors.text.tertiary, fontSize: 12, fontFamily: fonts.semiBold },
  ...
  menuContainer: { position: 'absolute', backgroundColor: colors.surface, borderRadius: 20, padding: 8, width: 220, borderWidth: 1, borderColor: colors.border, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20, elevation: 20 },
  ...
  actionIcon: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#2b2b2b', alignItems: 'center', justifyContent: 'center' },
  actionLabel: { fontSize: 14, fontFamily: fonts.medium, color: colors.text.primary },
});
